...
class AppConfig(object):
    def __init__(self):
        ...
        # list of all enabled zyrkd configurations (DashNetworkConnectionCfg) - they will be used accourding to
        # the order in list
        ...
        self.block_explorer_tx_mainnet = 'https://explorer.zyrk.io/tx/%TXID%'
        self.block_explorer_addr_mainnet = 'https://explorer.zyrk.io/address/%ADDRESS%'
        self.block_explorer_tx_testnet = 'https://explorer.zyrk.io/tx/%TXID%'
        self.block_explorer_addr_testnet = 'https://explorer.zyrk.io/address/%ADDRESS%'
        self.dash_central_proposal_api = 'https://zyrk.io/api/v1/proposal?hash=%HASH%'
    ...
    def init(self, app_path):
        ...
        self.log_file = os.path.join(self.log_dir, 'masternode-tool.log')
    ...
    def read_from_file(self, hw_session: HwSessionInfo, file_name: Optional[str] = None,
        ...
        try:
            cfgs = self.decode_connections(default_config.zyrkd_default_connections)
            if cfgs:
...
class DashNetworkConnectionCfg(object):
    ...
    def is_http_proxy(self):
        ...
        proxy uses SSL while normal, "local" zyrkd does not. 
        """
        if self.__use_ssl:
            return True
        else:
