...
from zyrkd_intf import DashdInterface
...
class ConfigDlg(QDialog, Ui_ConfigDlg, WndUtils):
    ...
    def on_btnRestoreDefault_clicked(self):
        if self.queryDlg('Do you really want to restore default connection(s)?',
                         buttons=QMessageBox.Yes | QMessageBox.Cancel,
                         default_button=QMessageBox.Yes, icon=QMessageBox.Information) == QMessageBox.Yes:
            cfgs = self.local_config.decode_connections(default_config.zyrkd_default_connections)
            if cfgs:
                # update the main list containing connections configuration from separate  lists dedicated
                # to mainnet and testnet - it'll be used by the import_connection method
                self.local_config.dash_net_configs.clear()
    ...
    def on_btnSshReadRpcConfig_clicked(self):
        ...
        if self.current_network_cfg:
            ...
            if not ok or not username:
                return
            from zyrkd_intf import DashdSSH
            ssh = DashdSSH(host, int(port), username)
            try:
                ssh.connect()
                zyrkd_conf = ssh.find_zyrkd_config()
                self.disable_cfg_update = True
                if isinstance(zyrkd_conf, tuple) and len(zyrkd_conf) >= 3:
                    if not zyrkd_conf[0]:
                        self.infoMsg('Remote Zyrk daemon seems to be shut down')
                    elif not zyrkd_conf[1]:
                        self.infoMsg('Could not find remote zyrk.conf file')
                    else:
                        file = zyrkd_conf[2]
                        ...
                        if file.get('server', '1') == '0':
                            self.warnMsg("Remote zyrk.conf parameter 'server' is set to '0', so RPC interface will "
                                         "not work.")
                        if not rpcuser:
                            self.warnMsg("Remote zyrk.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                         "not work.")
                        if not rpcpassword:
                            self.warnMsg("Remote zyrk.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                         "not work.")
                    self.update_connection_details_ui()
                elif isinstance(zyrkd_conf, str):
                    self.warnMsg("Couldn't read remote zyrkd configuration file due the following error: " +
                                 zyrkd_conf)
                ssh.disconnect()
            except Exception as e:
                self.errorMsg(str(e))
    ...
    def on_btnTestConnection_clicked(self):
        if self.current_network_cfg:
            ...
            zyrkd_intf = DashdInterface(window=self)
            zyrkd_intf.initialize(self.local_config, connection=self.current_network_cfg,
                                  for_testing_connections_only=True)
            try:
                info = zyrkd_intf.getinfo(verify_node=True)
                if info:
