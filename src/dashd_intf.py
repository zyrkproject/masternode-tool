...
class DashdSSH(object):
    ...
    def find_zyrkd_config(self):
        """
        Try to read configuration of remote dash daemon. In particular we need parameters concering rpc
        configuration.
        :return: tuple (zyrkd_running, zyrkd_config_found, zyrkd config file contents as dict)
                or error string in error occured
        """
        zyrkd_running = False
        zyrkd_config_found = False
        if not self.ssh:
            raise Exception('SSH session not ready')
        try:
            # find zyrkd process id if running
            try:
                pids = self.remote_command('ps -C "zyrkd" -o pid')
            except UnknownError:
                raise Exception('is zyrkd running on the remote machine?')
            pid = None
            ...
            if pid:
                zyrkd_running = True
                # using zyrkd pid find its executable path and then .zyrk directory and finally zyrk.conf file
                ...
                if executables and len(executables) >= 1:
                    ...
                    if len(elems) == 2:
                        ...
                        zyrkd_config_found = True
            return zyrkd_running, zyrkd_config_found, config
        except Exception as e:
            return str(e)
...
class DashdIndexException(JSONRPCException):
    ...
    def __init__(self, parent_exception):
        ...
        self.message = self.message + \
                       '\n\nMake sure the zyrkd daemon you are connecting to has the following options enabled in ' \
                       'its zyrk.conf:\n\n' + \
                       'addressindex=1\n' + \
                       'spentindex=1\n' + \
                       'timestampindex=1\n' + \
                       'txindex=1\n\n' + \
                       'Changing these parameters requires to execute zyrkd with "-reindex" option (linux: ./zyrkd -reindex)'
            ...
            for try_nr in range(1, 5):
                try:
                    try:
                        ...
                        # exceptions raised most likely by not functioning zyrkd node; try to switch to another node
...
class DashdInterface(WndUtils):
    ...
    def switch_to_next_config(self):
        """
        If there is another zyrkd config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        ...
        try:
            if not self.cur_conn_def:
                raise Exception('There is no connections to Zyrk network enabled in the configuration.')
            ...
            while True:
                try:
                    ...
                    return False
                        ...
                        NoValidConnectionsError) as e:
                    # exceptions raised by not likely functioning zyrkd node; try to switch to another node
                    # if there is any in the config
                    if not self.switch_to_next_config():
        ...
        if self.open():
            ...
            if verify_node:
                ...
                if self.config.is_testnet() and not node_under_testnet:
                    raise Exception('This RPC node works under Zyrk MAINNET, but your current configuration is '
                                    'for TESTNET.')
                elif self.config.is_mainnet() and node_under_testnet:
                    raise Exception('This RPC node works under Zyrk TESTNET, but your current configuration is '
                                    'for MAINNET.')
            return info
        else:
            raise Exception('Not connected')
    ...
    def issynchronized(self):
        if self.open():
            ...
            if self.cur_conn_def.is_http_proxy():
                ...
                syn = self.proxy.mastersync('status')
    ...
    def mastersync(self):
        if self.open():
            ...
            if self.cur_conn_def.is_http_proxy():
                ...
                return self.proxy.mastersync('status')
        else:
            raise Exception('Not connected')
    ...
    def masternodebroadcast(self, what, hexto):
        if self.open():
            # if what == 'relay':
            if False:
                # FIXME: relay does not report correct status without 3rd parameter due to bug in zyrkd
                return self.proxy.masternodebroadcast(what, hexto, "not-safe")
            else:
            ...
            if len(args) == 1 and args[0] == 'full':
                ...
                if self.masternodes and data_max_age > 0 and \
                   int(time.time()) - last_read_time < data_max_age:
                    ...
                    logging.info('Loading masternode list from Zyrk daemon...')
