...
from zyrkd_intf import DashdIndexException
...
class FindCollateralTxDlg(QDialog, ui_find_coll_tx_dlg.Ui_FindCollateralTxDlg, wnd_utils.WndUtils):
    def __init__(self, parent, zyrkd_intf, dash_address, read_only):
    ...
    def display_utxos(self):
        ...
        if len(self.utxos):
            ...
            self.setBaseSize(sh)
            if self.read_only:
                msg = f'<span style="color:blue">Found 7500 Zyrk transaction(s):' \
                      f'</span>'
            else:
                msg = f'<span style="color:blue">Found 7500 Zyrk transaction(s). Click the "Apply" button to copy' \
            ...
            self.lblMessage.setText('<span style="color:red">Found no unspent 7500 Zyrk transactions  '
