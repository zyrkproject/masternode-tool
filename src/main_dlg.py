...
from zyrkd_intf import DashdInterface, DashdIndexException
...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def __init__(self, app_path):
        ...
        self.zyrkd_intf = DashdInterface(window=None,
        ...
        self.hw_session = HwSessionInfo(
            ...
            self.config,
            zyrkd_intf=self.zyrkd_intf)

        self.zyrkd_info = {}
        self.is_zyrkd_syncing = False
        self.zyrkd_connection_ok = False
        self.connecting_to_zyrkd = False
    ...
    def setupUi(self):
        ...
        self.zyrkd_intf.initialize(self.config)
    ...
    def load_configuration_from_file(self, file_name) -> None:
        ...
        try:
            ...
            self.editing_enabled = False
            self.configuration_to_ui()
            self.zyrkd_intf.reload_configuration()
    ...
    def check_for_updates_thread(self, ctrl, force_check):
        ...
        try:
            ...
            response = urllib.request.urlopen(
                'https://raw.githubusercontent.com/zyrkproject/masternode-tool/release/version.txt')
    ...
    def show_connection_initiated(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> trying %s...' % self.zyrkd_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.setStatus1Text('<b>RPC network status:</b> failed connection to %s' % self.zyrkd_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Dash RPC node."""
        self.setStatus1Text('<b>RPC network status:</b> OK (%s)' % self.zyrkd_intf.get_active_conn_description(), 'green')
    ...
    def checkDashdConnection(self, wait_for_check_finish=False, call_on_check_finished=None):
        ...
        def wait_for_synch_finished_thread(ctrl):
            ...
            try:
                ...
                while not ctrl.finish:
                    synced = self.zyrkd_intf.issynchronized()
                    if synced:
                        self.is_zyrkd_syncing = False
                        self.show_connection_successful()
                        break
                    mastersync = self.zyrkd_intf.mastersync()
                    self.setmessage('Zyrk daemon is synchronizing: assetid: %s, assetname: %s' %
                                        (str(mastersync.get('AssetID', '')),
                                         str(mastersync.get('AssetName', ''))
                ...
                mtx.unlock()
                self.wait_for_zyrkd_synced_thread = None
        ...
        def connect_thread(ctrl):
            ...
            try:
                synced = self.zyrkd_intf.issynchronized()
                self.zyrkd_info = self.zyrkd_intf.getinfo(verify_node=True)
                self.zyrkd_connection_ok = True
                if not synced:
                    logging.info("zyrkd not synced")
                    if not self.is_zyrkd_syncing and not (hasattr(self, 'wait_for_zyrkd_synced_thread') and
                                                                  self.wait_for_zyrkd_synced_thread is not None):
                        self.is_zyrkd_syncing = True
                        self.wait_for_zyrkd_synced_thread = self.run_thread(self, wait_for_synch_finished_thread, (),
        ...
        if self.config.is_config_complete():
            if not hasattr(self, 'check_conn_thread') or self.check_conn_thread is None:
                ...
                if hasattr(self, 'wait_for_zyrkd_synced_thread') and self.wait_for_zyrkd_synced_thread is not None:
                    if call_on_check_finished is not None:
                        # if a thread waiting for zyrkd to finish synchronizing is running, call the callback function
                        call_on_check_finished()
                else:
                    self.connecting_to_zyrkd = True
    ...
    def on_action_check_network_connection_triggered(self):
        def connection_test_finished():
            ...
            if self.zyrkd_connection_ok:
                if self.is_zyrkd_syncing:
                    self.infoMsg('Connection successful, but Zyrk daemon is synchronizing.')
                else:
                    self.infoMsg('Connection successful.')
            else:
                if self.zyrkd_intf.last_error_message:
                    self.errorMsg('Connection error: ' + self.zyrkd_intf.last_error_message)
                else:
        ...
        if self.config.is_config_complete():
            ...
            self.errorMsg("There are no (enabled) connections to an RPC node in your configuration. "
                    "Please go to Settings and setup RPC connection to your zyrkd.")
            ...
            if self.curMasternode and self.curMasternode.collateralAddress:
                ...
                if not user_cancelled:
                    if not paths or len(paths) == 0:
                        self.errorMsg("Couldn't find Zyrk address in your hardware wallet. If you are using HW passphrase, "
    ...
    def on_btnBroadcastMn_clicked(self):
        ...
        if not self.zyrkd_connection_ok:
            self.errorMsg("Connection to Zyrk daemon is not established.")
            return
        if self.is_zyrkd_syncing:
            self.warnMsg("Zyrk daemon to which you are connected is synchronizing. You have to wait "
                         "until it's finished.")
            return
        ...
        try:
            ...
            seq = 0xffffffff
            block_count = self.zyrkd_intf.getblockcount()
            block_hash = self.zyrkd_intf.getblockhash(block_count - 12)
            ...
            if not cfg_collateral_address:
                ...
                if self.queryDlg(message="The Zyrk address retrieved from the hardware wallet (%s) for the configured "
            ...
            try:
                utxos = self.zyrkd_intf.getaddressutxos([hw_collateral_address])
                found = False
                utxo = []
                for utxo in utxos:
                    if utxo['txid'] == self.curMasternode.collateralTx and \
                       str(utxo['outputIndex']) == self.curMasternode.collateralTxIndex:
                        ...
                        break
                if found:
                    if utxo.get('satoshis', None) != 750000000000:
                        if self.queryDlg(
                                message="Collateral transaction output should equal 750000000000 Zyrkites (7500 Zyrk)"
                                        ", but its value is: %d Zyrkites.\n\nDo you really want to continue?"
                                        % (utxo['satoshis']),
            ...
            info = self.zyrkd_intf.getinfo(verify_node=True)
            ...
            ret = self.zyrkd_intf.masternodebroadcast("decode", work)
            if ret['overall'].startswith('Successfully decoded broadcast messages for 1 masternodes'):
                ...
                retried = False
                while True:
                    ret = self.zyrkd_intf.masternodebroadcast("relay", work)
                    ...
                    if failed_count:
                        if not retried:
                            # Due to zyrkd bug first `masternodebroadcast relay` can report failure while it actually
                    ...
                    if failed_count == 0:
                        ...
                        self.errorMsg('Failed to start masternode.\n\nResponse from Zyrk daemon: %s.' % errorMessage)
                    break
            else:
    ...
    def get_masternode_status(self, masternode):
        ...
        if self.zyrkd_connection_ok:
            ...
            mns_info = self.zyrkd_intf.get_masternodelist('full', collateral_id)
            if len(mns_info):
    ...
    def get_masternode_status_description(self):
        ...
        if self.zyrkd_connection_ok:
            ...
            mns_info = self.zyrkd_intf.get_masternodelist('full', data_max_age=120)  # read new data from the network
                                                                                     # every 120 seconds
            mn_info = self.zyrkd_intf.masternodes_by_ident.get(collateral_id)
            if mn_info:
                ...
                if mn_info.status == 'ENABLED' or mn_info.status == 'PRE_ENABLED':
                    ...
                    color = 'red'
                enabled_mns_count = len(self.zyrkd_intf.payment_queue)
                ...
                if addr:
                    try:
                        bal = self.zyrkd_intf.getaddressbalance([addr])
                        if bal:
            ...
            status = '<span style="color:red">Problem with connection to zyrkd.</span>'
        return status
    ...
    def on_action_transfer_funds_for_cur_mn_triggered(self):
        ...
        if self.curMasternode:
            ...
            if not self.curMasternode.collateralBip32Path:
                ...
                self.errorMsg("Enter the Masternode collateral Zyrk address. You can use the 'left arrow' "
    ...
    def on_btnFindCollateral_clicked(self):
        ...
        """
        if self.curMasternode and self.curMasternode.collateralAddress:
            ui = FindCollateralTxDlg(self, self.zyrkd_intf, self.curMasternode.collateralAddress,
    ...
    #     ui = ProposalsDlg(self, self.zyrkd_intf)
    #     ui.exec_()
