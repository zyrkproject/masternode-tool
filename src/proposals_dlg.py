...
from zyrkd_intf import DashdIndexException, Masternode
...
class VotingMasternode(AttrsProtected):
    def __init__(self, masternode, masternode_config):
        ...
        :param masternode: ref to an object storing mn information read from the network (zyrkd_intf.Masternode)
...
class ProposalsDlg(QDialog, ui_proposals.Ui_ProposalsDlg, wnd_utils.WndUtils):
    def __init__(self, parent, zyrkd_intf):
    ...
    def read_proposals_from_network(self):
        ...
        try:
            ...
            proposals_new = self.zyrkd_intf.gobject("list", "valid", "proposals")
            ...
            if len(proposals_new) > 0:
                        ...
                        for prop in self.proposals:
                            ...
                            if prop.marker:
                                if not prop.db_id:
                                    ...
                                    # zyrkd sometimes does not return some proposals, so they are deactivated id the db
                ...
                logging.warning('No proposals returned from zyrkd.')
    ...
    def read_governance_data(self):
        try:
            ...
            self.governanceinfo = self.zyrkd_intf.getgovernanceinfo()
            ...
            self.next_budget_amount = float(self.zyrkd_intf.getsuperblockbudget(sb_next))

            # superblocks occur every 16616 blocks (approximately 28.8 days)
            cur_block = self.zyrkd_intf.getblockcount()

            sb_last_hash = self.zyrkd_intf.getblockhash(sb_last)
            last_bh = self.zyrkd_intf.getblockheader(sb_last_hash)
            self.last_superblock_time = last_bh['time']
            self.next_superblock_time = 0
            if cur_block > 0 and cur_block <= sb_next:
                cur_hash = self.zyrkd_intf.getblockhash(cur_block)
                cur_bh = self.zyrkd_intf.getblockheader(cur_hash)
        ...
        try:
            ...
            if not self.zyrkd_intf.open():
                ...
                try:
                    ...
                    mns = self.zyrkd_intf.get_masternodelist('full')
                ...
                if not self.zyrkd_intf.open():
                    ...
                    try:
                        ...
                        for row_idx, prop in enumerate(proposals):
                            ...
                            tm_begin = time.time()
                            votes = self.zyrkd_intf.gobject("getvotes", prop.get_value('hash'))
