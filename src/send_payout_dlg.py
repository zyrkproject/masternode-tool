...
class WalletDlg(QDialog, ui_send_payout_dlg.Ui_SendPayoutDlg, WndUtils):
    ...
    def __init__(self, main_ui, initial_mn_sel: int):
        ...
        self.zyrkd_intf: DashdInterface = main_ui.zyrkd_intf
    ...
    def on_btnSend_clicked(self):
        ...
        if len(utxos):
            ...
            for utxo_idx, utxo in enumerate(utxos):
                ...
                if utxo['collateral']:
                    if self.queryDlg(
                            "Warning: you are going to transfer Masternode's collateral (7500 ZYRK) transaction "
                ...
                if addr_hw != utxo['address']:
                    self.errorMsg("<html style=\"font-weight:normal\">ZYRK address inconsistency between UTXO (%d) and HW path: %s.<br><br>"
                                 "<b>HW address</b>: %s<br>"
            ...
            try:
                ...
                if dest_data:
                    ...
                    if len(tx_hex) > 90000:
                        ...
                        tx_dlg = TransactionDlg(self, self.main_ui.config, self.zyrkd_intf, tx_hex, use_is)
                        if tx_dlg.exec_():
        ...
        def apply_utxos(new_utxos: List[dict], existing_utxos: List[dict]):
            ...
            if self.utxo_src_mode == 1:
                if self.mn_src_index is not None and self.mn_src_index < len(self.masternode_addresses) and \
                        self.mn_src_index >= 0:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} ZYRK&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} ZYRK&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.masternode_addresses[self.mn_src_index][0]}'
                    self.set_message_2(msg)
                else:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} ZYRK&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} ZYRK'
                    self.set_message_2(msg)
            elif self.utxo_src_mode == 2:
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} ZYRK&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} ZYRK'
                self.set_message_2(msg)
            elif self.utxo_src_mode in (1, 3):
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} ZYRK&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} ZYRK&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.hw_src_address}'
                self.set_message_2(msg)
    ...
    def load_utxos_thread(self, ctrl: CtrlObject, new_utxos_out, existing_utxos_out):
        ...
        if not self.zyrkd_intf.open():
            ...
            tm_begin = time.time()
            try:
                cur_block_height = self.zyrkd_intf.getblockcount()
                ...
                for addr_path_chunk in get_addresses_to_scan(self, ctrl, addr_scan_ctrl):
                    try:
                        ...
                        balance = self.zyrkd_intf.getaddressbalance(addr_chunk)
                        ...
                        if balance.get('balance') > 0:
                            ...
                            uxs = self.zyrkd_intf.getaddressutxos(addr_chunk)
                            utxos_count += len(uxs)
                            ...
                            for idx, utxo in enumerate(uxs):
                                ...
                                if not cached_utxo:
                                    ...
                                    try:
                                        ...
                                        rawtx = self.zyrkd_intf.getrawtransaction(utxo.get('txid'), 1)
                                        if rawtx:
